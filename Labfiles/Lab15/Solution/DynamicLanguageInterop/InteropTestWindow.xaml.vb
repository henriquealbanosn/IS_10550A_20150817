...
Class InteropTestWindow
    ...
    Private Sub ShuffleButton_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        Try
            ...
            ' Display the shuffled results in the ShuffledDataTextBox
    ...
    Private Sub ShuffleData(ByVal data() As Object)
        ...
        Dim paths As ICollection(Of String) = pythonEngine.GetSearchPaths()
        paths.Add(pythonLibPath)
        pythonEngine.SetSearchPaths(paths)

        Dim pythonScript As Object = pythonEngine.ExecuteFile(pythonCode)
        Dim pythonShuffler As Object = pythonScript.CreateShuffler()
    ...
    Private Sub VisualizeButton_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        Try
            Dim vertexAInDegrees As Integer = CType(VertexASlider.Value, Integer)
            Dim lengthSideAB As Integer = CType(SideABSlider.Value, Integer)
            Dim lengthSideCD As Integer = CType(SideCDSlider.Value, Integer)
            Dim heightOfTrapezoid As Integer = CType(HeightSlider.Value, Integer)

            Dim trapezoid As Object = CreateTrapezoid(vertexAInDegrees,
                lengthSideAB, lengthSideCD, heightOfTrapezoid)

            DisplayStatistics(trapezoid, Me.TrapezoidStatisticsTextBlock)

            RenderTrapezoid(trapezoid, Me.TrapezoidCanvas)
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    End Sub
    ...
    Private Sub DisplayStatistics(ByVal trapezoid As Object, ByVal trapezoidStatistics As TextBlock)
        ...
        builder.Append(String.Format(vbNewLine & "Area:" & vbTab & vbTab & "{0}", trapezoid.area().ToString()))
    ...
    Private Function CreateTrapezoid(ByVal vertexAInDegrees As Integer, ByVal lengthSideAB As Integer, ByVal lengthSideCD As Integer, ByVal heightOfTrapezoid As Integer) As Object
        Dim rubyRuntime As ScriptRuntime = Ruby.CreateRuntime()

        Dim rubyScript As Object = rubyRuntime.UseFile(rubyCode)

        Dim rubyTrapezoid As Object = rubyScript.CreateTrapezoid(
