...
#End Region
    ...
    Private Function BuildLambdaExpressionForQueryCriteria(ByVal dateRangeSpecified As Boolean, ByVal startDate As DateTime, ByVal endDate As DateTime,
        ...
        If dateRangeSpecified OrElse temperatureRangeSpecified OrElse appliedStressRangeSpecified OrElse deflectionRangeSpecified Then
            ...
            Dim testResultType As Type = GetType(TestResult)
            Dim itemBeingQueried As Expressions.ParameterExpression =
            ...
            Dim temperatureCondition As Expressions.BinaryExpression = Nothing
            Dim appliedStressCondition As Expressions.BinaryExpression = Nothing
            Dim deflectionCondition As Expressions.BinaryExpression = Nothing
            ...
            dateCondition = BuildDateExpressionBody(dateRangeSpecified, startDate, endDate,
            ...
            Dim body As Expressions.Expression = BuildLambdaExpressionBody(
            ...
            ' body expressions.
            lambda = Expressions.Expression.Lambda(Of Func(Of TestResult, Boolean))(body, itemBeingQueried)
        End If
...
#Region "Support methods used to build the dynamic lambda expression for specifying query criteria"
    ...
    Private Function BuildDateExpressionBody(ByVal dateRangeSpecified As Boolean, ByVal startDate As DateTime, ByVal endDate As DateTime, ByVal testResultType As Type, ByVal itemBeingQueried As Expressions.ParameterExpression) As Expressions.BinaryExpression
        ...
        If dateRangeSpecified Then
            ...
            '
            Dim testDateProperty As MemberInfo =
                testResultType.GetProperty("TestDate")

            Dim testDateMember As Expressions.MemberExpression =
                Expressions.Expression.MakeMemberAccess(
                itemBeingQueried, testDateProperty)


            Dim lowerDate As Expressions.ConstantExpression =
                Expressions.Expression.Constant(startDate)

            Dim lowerDateCondition As Expressions.BinaryExpression =
            ...
            '
            Dim upperDate As Expressions.ConstantExpression =
                Expressions.Expression.Constant(endDate)
            Dim upperDateCondition As Expressions.BinaryExpression =
    ...
    Private Function BuildNumericExpressionBody(ByVal rangeSpecified As Boolean, ByVal lowerRange As Short, ByVal upperRange As Short, ByVal testResultType As Type, ByVal propertyName As String, ByVal itemBeingQueried As Expressions.ParameterExpression) As Expressions.BinaryExpression
        ...
        If rangeSpecified Then
            ...
            '
            Dim testProperty As MemberInfo =
                testResultType.GetProperty(propertyName)

            Dim testMember As Expressions.MemberExpression =
                Expressions.Expression.MakeMemberAccess(
                itemBeingQueried, testProperty)

            Dim lowerValue As Expressions.ConstantExpression =
                Expressions.Expression.Constant(lowerRange)

            Dim lowerValueCondition As Expressions.BinaryExpression =
                Expressions.Expression.GreaterThanOrEqual(
            ...
            '
            Dim upperValue As Expressions.ConstantExpression =
                Expressions.Expression.Constant(upperRange)

            Dim upperValueCondition As Expressions.BinaryExpression =
                Expressions.Expression.LessThanOrEqual(
    ...
    Private Function BuildLambdaExpressionBody(ByVal dateCondition As Expressions.BinaryExpression, ByVal temperatureCondition As Expressions.BinaryExpression, ByVal appliedStressCondition As Expressions.BinaryExpression, ByVal deflectionCondition As Expressions.BinaryExpression) As Expressions.Expression
        ...
        If Not dateCondition Is Nothing Then
            body = dateCondition
        End If

        ' Add the temperatureCondition expression.
        If Not temperatureCondition Is Nothing Then
        ...
        If Not appliedStressCondition Is Nothing Then
            If body Is Nothing Then
        ...
        If Not deflectionCondition Is Nothing Then
            If body Is Nothing Then
...
#End Region
    ...
    Private Function BuildLambdaExpressionForOrderBy(ByVal obKey As OrderByKey) As Expressions.Expression(Of Func(Of TestResult, ValueType))
        ...
        If obKey <> OrderByKey.None Then
            ...
            ' specifies.
            Dim sortKey As Expressions.MemberExpression = Nothing
            Dim propty As MemberInfo = Nothing

            Select Case obKey
                Case OrderByKey.ByDate
                    ' If the user selected the date column, set the propty
                    ' object to TestDate.
                    propty = testResultType.GetProperty("TestDate")
                Case OrderByKey.ByTemperature
                    ' If the user selected the temperature column, set the 
                    ' propty object to Temperature.
                    propty = testResultType.GetProperty("Temperature")
                Case OrderByKey.ByAppliedStress
                    ' If the user selected the applied stress column, set the 
                    ' propty object to AppliedStress.
                    propty = testResultType.GetProperty("AppliedStress")
                Case OrderByKey.ByDeflection
                    ' If the user selected the deflection column, set the 
                    ' propty object to Deflection.
                    propty = testResultType.GetProperty("Deflection")
            End Select
            ...
            sortKey = Expressions.Expression.MakeMemberAccess(
                itemBeingQueried, propty)


            ' Cast the sortKey object to a ValueType object (ValueType is the 
            ' ancestor of all value types, including DateTime and short)
            Dim convert As Expressions.UnaryExpression =
                Expressions.Expression.Convert(sortKey, GetType(ValueType))

            ' Build the lambda expression by using the parameter and the 
            ' expression that contains the sort key
            lambda = Expressions.Expression.Lambda(Of Func(Of TestResult, ValueType))(convert, itemBeingQueried)
        End If
